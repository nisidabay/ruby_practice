...
OptionParser.new do |opts|
  ...
  opts.on("-c", "--count COUNT", Integer, "Number of items (default: 1)") do |count|
    if count < 0
      raise OptionParser::InvalidArgument, "Count cannot be negative"
    end
    options[:count] = count
  end
  ...
  opts.on("-f", "--format FORMAT", ["json", "xml", "yaml", "csv"], 
  ...
  opts.on("-w", "--wait [SECONDS]", Integer, "Wait time in seconds") do |seconds|
    if wait < 0
      raise OptionParser::InvalidArgument, "Wait cannot be negative"
    end
